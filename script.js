...
function calculatePressureHead(selectedUnit, head) {
  let pressureHead = 0.0;
  if (selectedUnit == 1) {
    pressureHead = head;
  } else if (selectedUnit == 2) {
    pressureHead = head * 0.305;
  }
...
function calculateEfficiencyOptimum(pumpType, f4_pump_flow_m3s) {
  ...
  if (pumpType === "A") {
    ...
    f9_efficiency_optimum =
      87.6345 + 2.0326 * ln_f4 - 1.4278 * Math.pow(ln_f4, 2);
  } else if (pumpType === "F") {
...
function calculateEfficiencyCorrection(pumpType, specificSpeed) {
  ...
  if (pumpType === "V") {
    if (specificSpeed <= 100) {
      f10_efficiency_correction =
        0.00000001875 * Math.pow(specificSpeed, 4) -
        0.00001219 * Math.pow(specificSpeed, 3) +
        0.002517 * Math.pow(specificSpeed, 2) -
        0.2123 * specificSpeed +
        6.4316;
    } else {
      // specificSpeed > 100
      f10_efficiency_correction =
        0.0000000006184 * Math.pow(specificSpeed, 4) -
        0.0000007899 * Math.pow(specificSpeed, 3) +
        0.0003441 * Math.pow(specificSpeed, 2) -
        0.0471 * specificSpeed +
        2.0453;
    }
  } else {
    // pumpType is not "V"
    if (specificSpeed < 30) {
      f10_efficiency_correction =
        -0.0002692 * Math.pow(specificSpeed, 3) +
        0.02558 * Math.pow(specificSpeed, 2) -
        0.9566 * specificSpeed +
        13.717;
    } else if (specificSpeed <= 90) {
      f10_efficiency_correction =
        -0.00002077 * Math.pow(specificSpeed, 3) +
        0.004611 * Math.pow(specificSpeed, 2) -
        0.3076 * specificSpeed +
        6.452;
    } else {
      // specificSpeed > 90
      f10_efficiency_correction =
        0.0000471 * Math.pow(specificSpeed, 2) +
        0.01879 * specificSpeed -
        0.9191;
    }
  }
...
function performCalculations(data) {
  try {
    ...
    console.log(specificSpeed, "specificspeed");
    ...
    const f9_efficiency_optimum = calculateEfficiencyOptimum(
      pumpType,
      f4_pump_flow_m3s
    );

    // Calculate F10: Efficiency Correction
    const f10_efficiency_correction = calculateEfficiencyCorrection(
      pumpType,
      specificSpeed
    );

    // Calculate F11: Efficiency Deviation
    const f11_efficiency_deviation =
      calculateEfficiencyDeviation(f4_pump_flow_m3s);

    // Calculate F12: Actual Efficiency
    const raw_actual_efficiency =
      f9_efficiency_optimum - f10_efficiency_correction;
    const f12_actual_efficiency_text = `${raw_actual_efficiency.toFixed(
      2
    )} ± ${f11_efficiency_deviation.toFixed(2)} %`;

    if (raw_actual_efficiency < 0) {
      throw new Error(
        "Calculated Optimum Efficiency is negative. Please check input values."
      );
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const pumpTypeDescriptionElem = document.getElementById(
    "pumpTypeDescription"
  );
  ...
  calculateBtn.addEventListener("click", () => {
    const pumpFlowValue = parseFloat(
      document.getElementById("pumpFlowValue").value
    );
    const selectedFlowUnit = document.querySelector(
      'input[name="flowUnit"]:checked'
    ).value;

    const selectedHeadUnit = document.querySelector(
      'input[name="headUnit"]:checked'
    ).value;
    const head = parseFloat(document.getElementById("head").value);
    const rpm = parseFloat(document.getElementById("rpm").value);
    const pumpType = document.getElementById("pumpType").value;
    const f4_pump_flow_m3s = calculatePumpFlowM3S(
      selectedFlowUnit,
      pumpFlowValue
    );

    const pressureHead = calculatePressureHead(selectedHeadUnit, head);
    console.log(pressureHead, selectedHeadUnit, head);
    const specificSpeed =
      (rpm * Math.pow(f4_pump_flow_m3s, 0.5)) / Math.pow(pressureHead, 0.75);

    console.log(specificSpeed, "soesds");
    // Basic validation
    if (isNaN(pumpFlowValue)) {
      showErrorModal("Please enter Pump Flow ");
      return;
    }

    if (isNaN(head)) {
      showErrorModal("Please enter Pressure Head ");
      return;
    }

    if (isNaN(rpm)) {
      showErrorModal("Please enter rotational velocity");
      return;
    }
    if (!pumpType) {
      showErrorModal("Please select Pump Type ");
      return;
    }
    ...
    try {
      ...
      document.getElementById("efficiencyOptimum").textContent =
        result.efficiencyOptimum.toFixed(2);
      document.getElementById("efficiencyCorrection").textContent =
        result.efficiencyCorrection.toFixed(2);
      document.getElementById("efficiencyDeviation").textContent =
        result.efficiencyDeviation.toFixed(2);
      document.getElementById("actualEfficiency").innerHTML =
        result.actualEfficiency; // Use innerHTML for ± symbol and %
    } catch (error) {
